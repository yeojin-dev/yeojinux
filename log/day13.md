...
* A20 게이트 활성화 코드 응용해 프로세서 리셋하는 함수 추가
...
void kReboot( void )
{
    ...
    // 입력 버퍼(0x60)에 데이터가 비어 있으면 출력 포트에 값을 쓰는 커맨드와 출력 포트 데이터 전송
    for( i = 0 ; i < 0xFFFF ; i++ )
    {
    ...
    // 커맨드 레지스터에(0x64)에 출력 포트 설정 커맨드(0xD1)를 전달
    kOutPortByte( 0x64, 0xD1 );
...
* `AssemblyUtility.asm` 파일 생성해 C 언어에 사용하는 어셈블리어 함수를 추가해 둠

### C 언어 커널 엔트리 포인트 파일 수정

* 엔트리 포인트의 뒷부분에 키보드 드라이버 함수 호출하는 코드 추가

### 빌드와 실행

* qemu는 키보드 LED를 지원하지 않기 때문에 qemu 환경에서는 LED가 동작하지 않음
* 현재의 키보드 드라이버는 프로세서가 키보드 컨트롤러를 계속 확인해야만 하는 비효율이 존재하며 이를 해결하기 위해서는 인터럽트 방식의 처리가 필요함
