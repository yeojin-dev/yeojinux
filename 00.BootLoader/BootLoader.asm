...
TOTALSECTORCOUNT:   dw 0x02 ; 부트 로더를 제외한 MINT64 OS 이미지의 크기, 최대 1152섹터(0x90000바이트)까지 가능
...
READDATA:                       ; 디스크를 읽는 코드의 시작
    ...
    ; 읽은 섹터는 ES:BX 위치에 저장됨
    add si, 0x0020                  ; 512(0x200)바이트만큼 읽었으므로, 이를 세그먼트 레지스터 값으로 변환
    mov es, si                      ; ES 세그먼트 레지스터에 더해서 어드레스를 한 섹터만큼 증가

    ; 한 섹터를 읽었으므로 섹터 번호를 증가시키고 마지막 섹터(18)까지 읽었는지 판단, 마지막 섹터가 아니면 섹터 읽기로 이동해서 다시 섹터 읽기 수행
...
.MESSAGELOOP:                   ; 메시지를 출력하는 루프
    mov cl, byte[ si ]          ; SI 레지스터가 가리키는 문자열 위치에서 한 문자를 CL 레지스터(CX 레지스터의 하위 1바이트)에 복사
...
; 데이터 영역
MESSAGE1:               db 'MINT64 OS Boot Loader Start~!!', 0  ; 출력할 메시지 정의, 마지막은 0으로 설정해서 문자열이 종료되었음을 알 수 있도록 함
